...
class _PerformancePageState extends State<PerformancePage> {
            ...
            child: Column(
              children: [
                FutureBuilder(
                  ...
                  builder: (context, snapshot) {
                    return ElevatedButton(
                      child: const Text('usar isolate principal'),
                ...
                FutureBuilder(
                  ...
                  builder: (context, snapshot) {
                    return ElevatedButton(
                        child: const Text('usar isolate secundario'),
                        style: ElevatedButton.styleFrom(elevation: 8.0),
                        onPressed:
                            snapshot.connectionState == ConnectionState.done
    ...
    var future = computeOnMainIsolate()
      ..then((_) {
        var snackBar = const SnackBar(
          content: Text('Estas usando el isolate principal!'),
        );
  ...
  void handleComputeOnSecondary(BuildContext context) {
    var future = computeOnSecondaryIsolate()
      ..then((_) {
        var snackBar = const SnackBar(
          content: Text('Estas usando el isolate secundario!'),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      });
